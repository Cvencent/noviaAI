...
const severityColors: Record<Severity, string> = {
  ...
  [Severity.NORMAL]: 'text-yellow-600 bg-yellow-50',
  [Severity.MINOR]: 'text-blue-600 bg-blue-50',
};
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-6">
      <div className="max-w-7xl mx-auto">
        <div className="mb-6">
          <h1 className="text-2xl font-bold text-gray-900">一致性检查</h1>
          <p className="text-gray-600 mt-1">智能检测文本中的人物、时间线、世界观等一致性问题</p>
        </div>
                  ...
                  <div className="space-y-2">
                    {rules
                      ...
                      .map(rule => (
                        <div
                          key={rule.id}
                          className="flex items-start justify-between p-2 rounded hover:bg-gray-50"
                        >
  ...
  return (
    <Card>
      <div className="space-y-4">
        <div className="flex justify-between items-start">
          <div>
            <h2 className="text-lg font-semibold text-gray-900">检查详情</h2>
        ...
        <div className="grid grid-cols-3 gap-4 p-4 bg-gray-50 rounded-lg">
          ...
          <div className="text-center">
            <div className="text-2xl font-bold text-yellow-600">{report.normalCount}</div>
            <div className="text-sm text-gray-500">一般</div>
          </div>
          <div className="text-center">
            <div className="text-2xl font-bold text-blue-600">{report.minorCount}</div>
            <div className="text-sm text-gray-500">轻微</div>
          </div>
        </div>

        {reportIssues.length > 0 ? (
          <div className="space-y-3">
            <h3 className="font-medium text-gray-900">发现问题</h3>
            {reportIssues.map((issue: ReportIssue, index: number) => (
              ...
              >
                ...
                <h4 className="font-medium text-gray-900 mt-2">{issue.title}</h4>
                ...
                {issue.location && (
                  <div className="mt-2 p-2 bg-white rounded text-xs">
                    ...
                    {issue.location.chapterTitle && (
                      <span className="text-gray-700">{issue.location.chapterTitle}</span>
                    )}
                    {issue.location.position && (
                      <span className="text-gray-500"> · {issue.location.position}</span>
                    )}
                  </div>
                )}
          ...
          <div className="text-center py-8">
            ...
            <p className="text-gray-600">未发现一致性问题</p>
          </div>
        )}
