...
export function generateStylePrompt(
  ...
  let effectiveDialogueRatio = language.dialogue_ratio
  let effectivePacing = narrative.pacing
  let effectiveVocabulary = language.vocabulary_level
  let effectiveDescription = language.description_type

  if (tuningParams) {
    effectiveDialogueRatio = tuningParams.dialogue_ratio
    effectivePacing = tuningParams.pacing < 33 ? 'slow' : tuningParams.pacing < 66 ? 'moderate' : 'fast'
    effectiveVocabulary = tuningParams.vocabulary_level < 33 ? 'simple' : tuningParams.vocabulary_level < 66 ? 'intermediate' : 'advanced'
    effectiveDescription = tuningParams.description_detail < 33 ? 'minimal' : tuningParams.description_detail < 66 ? 'visual' : 'rich'
  }

  let prompt = `请以${name}的风格来写作。

## 叙事要求
- 视角: ${narrative.perspective === 'first_person' ? '第一人称（我/我们）' : narrative.perspective === 'third_person_limited' ? '第三人称限知' : '第三人称全知'}
- 时态: ${narrative.tense === 'past' ? '过去时' : narrative.tense === 'present' ? '现在时' : '混合时态'}
- 叙述口吻: ${narrative.voice === 'casual' ? '随意口语化' : narrative.voice === 'literary' ? '文学化' : narrative.voice === 'formal' ? '正式严肃' : narrative.voice === 'conversational' ? '对话感强' : '诗意化'}
- 整体基调: ${narrative.tone.join('、')}
- 节奏: ${effectivePacing === 'slow' ? '缓慢细腻' : effectivePacing === 'moderate' ? '中等节奏' : effectivePacing === 'fast' ? '快速紧凑' : '多变节奏'}

## 语言风格
- 词汇难度: ${effectiveVocabulary === 'simple' ? '简单易懂' : effectiveVocabulary === 'intermediate' ? '中等难度' : effectiveVocabulary === 'advanced' ? '高级词汇' : '学术化'}
- 句式结构: ${language.sentence_structure === 'short' ? '短句有力' : language.sentence_structure === 'mixed' ? '长短句结合' : language.sentence_structure === 'complex' ? '复杂长句' : '行云流水'}
- 段落长度: ${language.paragraph_length === 'concise' ? '简短精炼' : language.paragraph_length === 'moderate' ? '中等长度' : language.paragraph_length === 'descriptive' ? '详细描述' : '史诗级'}
- 对话比例: ${Math.round(effectiveDialogueRatio * 100)}% 对话, ${Math.round((1 - effectiveDialogueRatio) * 100)}% 叙述
- 描写侧重: ${effectiveDescription === 'visual' ? '视觉画面' : effectiveDescription === 'sensory' ? '多种感官' : effectiveDescription === 'emotional' ? '情感内心' : effectiveDescription === 'minimal' ? '极简留白' : '丰富细节'}
- 用词偏好: ${language.diction.join('、')}
- 常用修辞: ${language.figurative_language.join('、')}

## 标志性特征
- 标志性元素: ${style_analysis.signature_elements.join('、')}
- 叙事技巧: ${style_analysis.narrative_techniques.join('、')}
- 人物关注点: ${style_analysis.character_focus}
- 主题偏好: ${style_analysis.theme_preferences.join('、')}
- 情感共鸣: ${style_analysis.emotional_resonance}
- 节奏模式: ${style_analysis.pacing_pattern}

## 类型规范
- 预期套路: ${genre_conventions.expected_tropes.join('、')}
- 禁忌主题: ${genre_conventions.taboo_subjects.length > 0 ? genre_conventions.taboo_subjects.join('、') : '无'}
- 目标读者: ${genre_conventions.audience === 'young_adult' ? '青少年' : genre_conventions.audience === 'new_adult' ? '青年' : genre_conventions.audience === 'adult' ? '成人' : '全年龄'}
...
export function searchStyles(query: string): WritingStylePreset[] {
  const lowerQuery = query.toLowerCase()
  return WRITING_STYLE_PRESETS.filter(style => 
    style.name.toLowerCase().includes(lowerQuery) ||
    style.description.toLowerCase().includes(lowerQuery) ||
    style.recommended_genres.some(genre => genre.toLowerCase().includes(lowerQuery)) ||
    style.style_analysis.theme_preferences.some(theme => theme.toLowerCase().includes(lowerQuery))
  )
}
...
export function getPresetStyleConfig(id: string): WritingStylePreset | null {
  const preset = getStylePreset(id)
  return preset || null
}

export function applyTuningToPreset(
  ...
  return {
    ...
    language: {
      ...preset.language,
      dialogue_ratio: tuningParams.dialogue_ratio,
      description_type: tuningParams.description_detail < 33 ? 'minimal' 
        : tuningParams.description_detail < 66 ? 'visual' 
        : 'rich' as any,
      vocabulary_level: tuningParams.vocabulary_level < 33 ? 'simple' 
        : tuningParams.vocabulary_level < 66 ? 'intermediate' 
        : 'advanced' as any
    },
    narrative: {
      ...preset.narrative,
      pacing: tuningParams.pacing < 33 ? 'slow' 
        : tuningParams.pacing < 66 ? 'moderate' 
        : 'fast' as any
    }
  }
}
